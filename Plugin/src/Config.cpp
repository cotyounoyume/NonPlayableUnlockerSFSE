...
namespace Config
{
	...
	void ReadIni()
	{
		...
		if (!items) {
			Utility::Notification(fmt::format("ERROR: can't load toml file:{}", path.string()));
			return;
		}
		...
		for (const auto& item : *items) {
			if (!item.is_string()) {
				Utility::Notification(fmt::format("  toml format error: skip"));
				continue;
			}
			FormIDs.push_back(item.value_or(""));
			Utility::Notification(fmt::format("  toml element: {}", item.value_or("")));
				//item.as_integer());
		}
	...
	void Apply()
	{
		for (auto formID : FormIDs) {
			...
			Utility::Notification(fmt::format("form: {}'s formtype is {}. playable-flag is {}", form->GetFormEditorID(), form->GetFormType(), form->GetPlayable()));
		}
	}
}
